...
#if IS_ENABLED(CONFIG_NICE_OLED_WIDGET_ANIMATION_PERIPHERAL_ROTATE)
/* Rotate through a set of animations, advancing one step on every power-on.
   The current position persists in NVS via the settings subsystem. */
#include <zephyr/settings/settings.h>

/* Direct declarations: the head/pokemon/spaceman asset headers all share the
   same include guard, so only one of them can ever be #included. */
...
LV_IMG_DECLARE(pbongo_0);
LV_IMG_DECLARE(pbongo_1);
LV_IMG_DECLARE(pbongo_2);
LV_IMG_DECLARE(pbongo_3);
...
static const lv_img_dsc_t *rot_pokemon_imgs[] = {
...
static const lv_img_dsc_t *rot_spaceman_imgs[] = {
...
static const struct panim_entry panim_entries[] = {
    {crystal_imgs, 16, 960},        /* gem */
    {rot_head_imgs, 16, 4800},      /* rotating head */
...
static int panim_settings_set(const char *name, size_t len, settings_read_cb read_cb,
                              void *cb_arg) {
    const char *next;
    if (settings_name_steq(name, "idx", &next) && !next && len == sizeof(panim_idx)) {
        read_cb(cb_arg, &panim_idx, sizeof(panim_idx));
    }
    return 0;
}
SETTINGS_STATIC_HANDLER_DEFINE(panim, "panim", NULL, panim_settings_set, NULL, NULL);
#endif /* IS_ENABLED(CONFIG_SETTINGS) */
#endif /* IS_ENABLED(CONFIG_NICE_OLED_WIDGET_ANIMATION_PERIPHERAL_ROTATE) */
...
#if IS_ENABLED(CONFIG_NICE_OLED_WIDGET_ANIMATION_PERIPHERAL_ROTATE)
#if IS_ENABLED(CONFIG_SETTINGS)
    settings_load_subtree("panim");
    uint32_t panim_next = (panim_idx + 1) % ARRAY_SIZE(panim_entries);
    settings_save_one("panim/idx", &panim_next, sizeof(panim_next));
#endif
    const struct panim_entry *entry = &panim_entries[panim_idx % ARRAY_SIZE(panim_entries)];
    lv_animimg_set_src(art, (const void **)entry->imgs, entry->count);
    lv_animimg_set_duration(art, entry->duration_ms);
#else /* !ROTATE: single compile-time animation */
#if IS_ENABLED(CONFIG_NICE_OLED_WIDGET_ANIMATION_PERIPHERAL_HEAD)
