...
static void draw_name(lv_obj_t *canvas) {
    if (sizeof(CONFIG_NICE_OLED_WIDGET_PERIPHERAL_NAME) > 1) {
        lv_draw_label_dsc_t label_dsc;
        init_label_dsc(&label_dsc, LVGL_FOREGROUND, &pixel_operator_mono_16,
                       LV_TEXT_ALIGN_CENTER);
        lv_canvas_draw_text(canvas, 0, CONFIG_NICE_OLED_WIDGET_PERIPHERAL_NAME_Y, CANVAS_WIDTH,
                            &label_dsc, CONFIG_NICE_OLED_WIDGET_PERIPHERAL_NAME);
    }
}
